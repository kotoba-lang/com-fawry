(ns fawry.main
  "Kotodama WASM entrypoint for the Fawry clean-room actor (L4) — Clojure port.
  ...
  facts (`fawry.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fawry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fawry-compat" :tier tier :entities entities} 200])
